...
#include "drv_timer5_stm32f7xx.h"
...
static TIM_HandleTypeDef DrvTimer5;
...
const CO_IF_TIMER_DRV STM32F7xxTimer5Driver = {
    DrvTimerInit,
    DrvTimerReload,
    DrvTimerDelay,
    DrvTimerStop,
...
static void DrvTimerInit(uint32_t freq)
{
     ...
     */
    DrvTimer5.Instance = TIM5;
    DrvTimer5.Init.Prescaler = 108;
    DrvTimer5.Init.CounterMode = TIM_COUNTERMODE_UP;
    DrvTimer5.Init.Period = 4294967295;
    DrvTimer5.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    DrvTimer5.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
    if (HAL_TIM_Base_Init(&DrvTimer5) != HAL_OK) {
        while(1);    /* error not handled */
    }
    sClockSourceConfig.ClockSource = TIM_CLOCKSOURCE_INTERNAL;
    if (HAL_TIM_ConfigClockSource(&DrvTimer5, &sClockSourceConfig) != HAL_OK) {
        while(1);    /* error not handled */
    }
    sMasterConfig.MasterOutputTrigger = TIM_TRGO_RESET;
    sMasterConfig.MasterSlaveMode = TIM_MASTERSLAVEMODE_DISABLE;
    if (HAL_TIMEx_MasterConfigSynchronization(&DrvTimer5, &sMasterConfig) != HAL_OK) {
        while(1);    /* error not handled */
    }

    /* clear pending update interrupt */
    __HAL_TIM_CLEAR_FLAG(&DrvTimer5, TIM_FLAG_UPDATE);
...
static void DrvTimerStart(void)
{
    ...
    HAL_TIM_Base_Start_IT(&DrvTimer5);
}
...
static uint32_t DrvTimerDelay(void)
{
    uint32_t current = __HAL_TIM_GET_COUNTER(&DrvTimer5);
    uint32_t reload = __HAL_TIM_GET_AUTORELOAD(&DrvTimer5);
...
static void DrvTimerReload(uint32_t reload)
{
    ...
    __HAL_TIM_SET_AUTORELOAD(&DrvTimer5, reload);
}

static void DrvTimerStop(void)
{
    /* stop the hardware timer counting */
    HAL_TIM_Base_Stop_IT(&DrvTimer5);
}
